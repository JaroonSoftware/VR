...
function QuotationManage() {
  ...
  const handleBanksRemove = (record) => {
    ...
    const newData = banks.filter((item) => item?.acc_no !== record?.acc_no);
  ...
  const handleQuotationDetailEdit = (record) => {
    setManageConfig((s) => ({
      ...s,
      title: "Edit Quotation detail list",
      action: "edit",
      data: record,
    }));
  ...
  const prodcolumns = productColumn({
    handleRemove: handleQuotationDetailRemove,
    handleEdit: handleQuotationDetailEdit,
  });
      ...
      <Space size="small" direction="vertical" className="flex gap-2">
        <Row gutter={[8, 8]} className="m-0" align="middle">
          <Col xs={24} sm={24} md={12} lg={12} xl={12} xxl={12}>
            <Typography.Title level={4} className="m-0 !text-zinc-400">
              ธนาคารสำหรับใบเสนอราคา
            </Typography.Title>
          </Col>
          <Col xs={24} sm={24} md={12} lg={12} xl={12} xxl={12}>
            <Flex justify="end">
              <Button
                className="bn-success-outline bn-center"
                icon={<BankFilled />}
                ...
                onClick={() => {
                  form
                    ...
                    .then((v) => {
                      setManageConfig((s) => ({
                        ...s,
                        title: "Adding Quotation detail list",
                        action: "create",
                      }));
                      setOpenProducts(true);
                    })
