...
import { columns } from "./modal-unit.model";
...
const unservice =  UnitService();
const optionService = OptionService();
export default function ModalUnit({show, close, values}) {
    const [form] = useForm();
    ...
    const handleSubmit = (v) => {
        ...
        const action = (v) => !v?.id ? unservice.create({...v}) : unservice.update({...v});
    ...
    const handleSearch = (value) => {
        if(!!value){    
            const f = modalData.filter( d => ( d.unit?.toLowerCase().includes(value?.toLowerCase()) ) );
    ...
    const handleDeleted = (record) => {
        setLoading(true);
        unservice.deleted(record?.id || 0).then( _ =>  {
            search();
        }).catch( err => {
            console.warn(err);
            const data = err?.response?.data;
    ...
    const column = columns({handleChoose, handleEdit, handleDeleted});

    const search = () =>{
        setLoading(true);
        optionService.optionsUnit().then((res) => {
            let { data } = res.data; 
            setModalData(data);
    ...
    return (
        ...
        >
            <Spin spinning={loading}>
                            ...
                            <Row gutter={[{xs:32, sm:32, md:32, lg:12, xl:12}, 8]} className='m-0'>
                                <Col span={24}>
                                    <Typography.Link onClick={()=>{ setManageInit({}); setOpenManage(true); }} className='ps-1'>
                                        <span className='hover:underline underline-offset-1'>Create Unit</span>
                                    </Typography.Link> 
                                </Col>
                            </Row>
                        </Form>
                ...
                >
                    <UnitManage submit={(val)=>handleSubmit(val)} initial={manageInit} /> 
                </Drawer>}
            </Spin>
        </Modal>
        </>
