...
import { columnsDetail } from "./modal-sample-request.model";
import SampleRequest from '../../../service/SampleRequest.service';
export default function ModalSampleRequestDetail({show, close, values, selected}) {
    ...
    const column = columnsDetail({handleChoose});

    const onload = () => {
        // debugger;
        setLoading(true);
        SampleRequestService.sampleRequestDetail()
        .then((res) => {
          let { status, data } = res;
          if (status === 200) {
