...
if ($_SERVER["REQUEST_METHOD"] == "GET"){
    ...
    try {  
        ...
        $coa = $stmt->fetchAll(PDO::FETCH_ASSOC);
        ...
        $sql = "select * from spparameter where spcode = :code"; 
        $stmt = $conn->prepare($sql); 
        if (!$stmt->execute([ 'code' => $code ])){
            $error = $conn->errorInfo(); 
            http_response_code(401);
        ...
        $parm = $stmt->fetchAll(PDO::FETCH_ASSOC); 
        ...
        echo json_encode(array("data"=>array( "header" => $header, "coa" => $coa, "parm" => $parm,  "allergen" => $allergen,)));
